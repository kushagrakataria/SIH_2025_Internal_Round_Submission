...
import { Shield, User, Lock, Phone, ArrowLeft, Eye, EyeOff, RefreshCw, AlertTriangle } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { useAuth } from "@/contexts/AuthContext";
import { firebaseHealthChecker, FirebaseStatus } from "@/utils/firebaseHealth";
...
const Login = () => {
  ...
  const checkFirebaseHealth = async () => {
    ...
    try {
      const status = await firebaseHealthChecker.checkOverallHealth();
      setFirebaseStatus(status);
      
      if (!status.overall) {
        toast({
          title: "Firebase Connection Issue",
          description: "Some features may not work properly. Checking connection...",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleNetworkRetry = async () => {
    setCheckingFirebase(true);
    try {
      const recovered = await firebaseHealthChecker.attemptNetworkRecovery();
      if (recovered) {
        await checkFirebaseHealth();
        toast({
          title: "Connection Restored",
          description: "Firebase connection has been restored.",
        });
      } else {
        toast({
          title: "Connection Failed",
          description: "Unable to restore Firebase connection. Please check your internet.",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  return (
    <div className="mobile-screen bg-gradient-to-br from-primary/5 via-background to-accent/5">
      ...
      <div className="mobile-container pt-8 pb-4">
        ...
        {firebaseStatus && !firebaseStatus.overall && (
          <Alert className="mb-4 border-destructive/50 text-destructive">
            <AlertTriangle className="h-4 w-4" />
            <AlertDescription className="flex items-center justify-between">
              <span>
                Firebase connection issues detected. 
                {!firebaseStatus.auth.available && " Authentication unavailable."}
                {!firebaseStatus.firestore.available && " Database unavailable."}
              </span>
              <Button
                variant="outline"
                size="sm"
